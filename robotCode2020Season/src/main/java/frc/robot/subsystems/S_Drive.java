...
public class S_Drive extends SubsystemBase {
  ...
  private PIDController leftPIDController_WPI = new PIDController(Constants.kPTraj, 0, 0); // velocity controllers only
                                                                                           // have P
  ...
  public S_Drive() {
    ...
    // diffDrive.setDeadband(0.02); // default deadband for differentialDrive is
    // 0.02
    ...
    pose = new Pose2d(); // i think this should be the same as that which is used in
                         // DifferentialDriveOdometry?
    odometry = new DifferentialDriveOdometry(getHeading()); // may want other constructor for enabling a starting
                                                            // position
  ...
  /**
   * Translates the sign of the joystick value to fit a 180 degree range of
   * allowed positive degrees. Likely won't actually have this in the competition
   * code, I just got annoyed with the format of negative values on the f310 (also
   * coding for fun).
   * 
   * @param joystick      The controller from which the axis is gotten.
   * @param axis          The axis from which the axis magnitude value from [-1,
   *                      1] is gotten.
   * @param minPosDegrees The reference point on a unit circle which indicates the
   *                      lower bound of a 180 degree interval that should be
   *                      marked as positive. only actual sensible values would be
   *                      0, 90, 180, or 270 (or periodic equivalents).
   * @return the raw axis magnitude with clarified sign/direction in the dimention
   *         of the given axis.
   */
  public double getTranslatedJoystickAxisValue(Joystick joystick, int axis, double minPosDegrees) {
    ...
    double axisAngle = Math.IEEEremainder(-joystick.getDirectionDegrees() + 90, 360); // given angle starting from
                                                                                      // positive y-axis positive in
                                                                                      // clockwise direction
  ...
  public void periodic() {
    ...
    pose = odometry.update( // note: if we want odometry after autonomyous then would need a seperate tare
                            // for robotInit
        getHeading(), encoderLeft.getPosition() - tareEncPositionL, encoderRight.getPosition() - tareEncPositionR);
  }
}
