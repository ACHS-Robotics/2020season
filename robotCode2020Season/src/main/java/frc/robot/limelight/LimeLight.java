...
public final class LimeLight {
    private final double _heightOfRobot = 0;
    private final double _angleOfCamera = 0;
    ...
    /**
     * Determines whether the target is found or not. This is represented by the
     * "tv" table in the Shuffleboard.
     * 
    ...
    /**
     * Gets the horizontal offset from the crosshair to the target. This can be
     * anywhere from -27 degrees to 27 degrees, inclusive.
     * 
    ...
    /**
     * Gets the vertical offset from the crosshair to the target. This can be
     * anywhere from -20.5 degrees to 20.5 degrees.
     * 
     ...
     * 
     * @return The distance, if it exists; otherwise, -1.
     */
    public double getDistance(double heightOfTarget) {
        if (!this.hasTarget()) {
            return -1;
        }
        double heightDiff = heightOfTarget - this._heightOfRobot;
        double camAndTargetAngle = Math.toRadians(this._angleOfCamera + this.getVerticalOffset());
        return heightDiff / Math.tan(camAndTargetAngle);
    }
}